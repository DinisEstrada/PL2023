...
def abre_json(f):
    ...
    colecao = abre_csv("csv/"+f)
    with open("json/"+nome_json, "w", encoding="utf-8") as ficheiro_json:
        json.dump(colecao, ficheiro_json, indent=4, ensure_ascii=False, separators=(",", ":"))

if __name__ == "__main__":
    abre_json("alunos.csv")
    abre_json("alunos2.csv")
    abre_json("alunos3.csv")
    abre_json("alunos4.csv")
    abre_json("alunos5.csv")